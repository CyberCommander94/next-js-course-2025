  ...
  return (
    <section className="flex flex-col items-center w-full h-full">
      <div className="flex justify-end items-center w-full">
        <Link
          href="/rackets/top-10"
          className="px-2 py-2 bg-transparent border border-foreground dark:border text-center hover:text-white hover:bg-foreground dark:text-white dark:border-input dark:hover:bg-input/50"
        >
        ...
        <Carousel className="w-full">
          <CarouselContent className="p-5 pr-1 box-border mr-1">
            {rackets.map((item) => (
              <CarouselItem
                key={item.id}
                className=" max-w-[300px] sm:basis-1/2 md:basis-1/3 xl:basis-1/4 2xl:basis-1/5 flex-shrink-0"
              >
